...
## Editing the PHY init code

The PHY initialization sequence is generated by the Synopsys PHY init tool. To customize it:

1. **Pull in the development files** (required once):
   ```bash
   make setup_phyinit_dev      # Copies userCustom/ and doc/ into sw/libs/
   ```
   This copies the `userCustom` folder and documentation from `$(SYNOPSYS_PHYINIT_PATH)` into `sw/libs/`.

2. **Edit** the files in `sw/libs/userCustom/`.

3. **Rebuild** the PHY init output:
   ```bash
   make build_phyinit          # Rebuilds using userCustom as CUSTDIR
   ```
   The generated `.txt` files land in `sw/libs/phyinit/` and `sw/libs/phyinit.c` is rebuilt automatically (it concatenates the template with the output).

4. **Push changes back** to the PHY init source (optional):
   ```bash
   make commit_phyinit_changes
   ```

Note: `first_time_setup` runs `setup_phyinit_dev` automatically.
